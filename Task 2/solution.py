...
def setup_seeds_manual(seed: int) -> None:
    """
...
class SWAGInference(object):
    ...
    def __init__(
        ...
        :param var_scale: Scaling factor for variance in full SWAG
        :param var_clamp: Clamping value for variance in full SWAG
        :param num_networks: Number of networks to train
        """
        ...
        self.networks = [CNN(in_channels=3, out_classes=6).to(device) for _ in range(self.num_networks)]
        self.network = None
        ...
        for network in self.networks:
            ...
            self.swag_mean.append(self._create_weight_copy())
        self.swag_squared_mean = []
        for network in self.networks:
            self.network = network
            self.swag_squared_mean.append(self._create_weight_copy())
        self.swag_num_models = [0 for _ in range(self.num_networks)]

        # Full SWAG
        # DONE(2): create attributes for SWAG-full
        #  Hint: check collections.deque
        self.swag_deviation_matrix = [{
            name: collections.deque(maxlen=self.max_rank_deviation_matrix)
            for name, _ in network.named_parameters()
        } for network in self.networks]

        # Calibration, prediction, and other attributes
        # DONE(2): create additional attributes, e.g., for calibration
    ...
    def update_swag_statistics(self) -> None:
        ...
        i = self.networks.index(self.network)
        ...
        for name, param in copied_params.items():
            ...
            param_sum = self.swag_num_models[i] * self.swag_mean[i][name] + param
            param_squared_sum = self.swag_num_models[i] * self.swag_squared_mean[i][name] + param ** 2
            self.swag_mean[i][name] = param_sum / (self.swag_num_models[i] + 1)
            self.swag_squared_mean[i][name] = param_squared_sum / (self.swag_num_models[i] + 1)
        ...
        if self.inference_mode == InferenceType.SWAG_FULL:
            ...
            if self.swag_num_models[i] > 0:
                for name, param in copied_params.items():
                    self.swag_deviation_matrix[i][name].append(param - self.swag_mean[i][name])

        self.swag_num_models[i] += 1
    ...
    def fit_swag_model(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        # DONE(2): Update SWAGScheduler instantiation if you decided to implement a custom schedule.
    ...
    def apply_calibration(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        if self.inference_mode == InferenceType.MAP:
            ...
            self._calibration_threshold = torch.tensor([0.0 for _ in range(6)])
            return
        ...
        self._calibration_threshold = torch.tensor([0.76, 0.63, 0.81, 0.67, 0.62, 0.82])
        print("Calibration threshold:", self._calibration_threshold)

        # DONE(2): perform additional calibration if desired.
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        assert len(model_predictions) == self.num_bma_samples * self.num_networks
        assert all(
        ...
        bma_probabilities = torch.mean(torch.stack(model_predictions), dim=0).detach().cpu()
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            mean_weights = self.swag_mean[i][name]
            var_weights = self.swag_squared_mean[i][name] - self.swag_mean[i][name] ** 2
            if self.inference_mode == InferenceType.SWAG_FULL:
                var_weights = self.var_scale * torch.clamp(var_weights, self.var_clamp)
            std_weights = torch.sqrt(var_weights)
            ...
            if self.inference_mode == InferenceType.SWAG_FULL:
                ...
                swag_deviation_matrix = torch.stack(list(self.swag_deviation_matrix[i][name]), dim=-1)
                z_full = torch.randn(swag_deviation_matrix.size(-1), device=device)
                std_weights_full = math.sqrt(self.var_scale / (self.max_rank_deviation_matrix - 1)) * swag_deviation_matrix
                sampled_weight += std_weights_full @ z_full
    ...
    def predict_labels(self, predicted_probabilities: torch.Tensor) -> torch.Tensor:
        ...
        # DONE(2): implement a different decision rule if desired
        return torch.where(
            label_probabilities >= self._calibration_threshold[max_likelihood_labels],
    ...
    def fit(
        ...
        for i, network in enumerate(self.networks):
            ...
            PRETRAINED_WEIGHTS_FILE = self.model_dir / f"map_weights_{i + 1}.pt"
            if USE_PRETRAINED_MODEL and PRETRAINED_WEIGHTS_FILE.exists():
                self.network.load_state_dict(torch.load(PRETRAINED_WEIGHTS_FILE, map_location=device))
                print("Loaded pretrained MAP weights from", PRETRAINED_WEIGHTS_FILE)
            else:
                self.fit_map_model(loader)
                torch.save(self.network.state_dict(), PRETRAINED_WEIGHTS_FILE)
                print("Saved pretrained MAP weights to", PRETRAINED_WEIGHTS_FILE)
    ...
    def predict_probabilities_map(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        return torch.mean(torch.stack(model_predictions), dim=0).detach().cpu()
...
class SWAGScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    def calculate_lr(self, current_epoch: float, previous_lr: float) -> float:
        ...
        """
        # DONE(2): Implement a custom schedule if desired
        return previous_lr

    # DONE(2): Add and store additional arguments if you decide to implement a custom scheduler
    def __init__(
        self,
...
def evaluate(
    ...
    for threshold in threshold_values:
        thresholded_predictions = torch.where(max_pred_probabilities <= threshold, -1 * torch.ones_like(argmax_pred_labels), argmax_pred_labels)
    ...
    cost = cost_function(predicted_labels, labels).item()
    print(f"Cost on validation data: {cost:.4f}")
