...
class Model(object):
    ...
    def __init__(self):
        ...
        kernel_matern = ConstantKernel(1.0, (1e-3, 1e3)) * Matern(length_scale=1.0, length_scale_bounds=(1e-6, 1e5), nu=2.1)
        kernel_linear = ConstantKernel(1.0, (1e-3, 1e3)) * DotProduct(sigma_0=1.0, sigma_0_bounds=(1e-3, 1e3))

        # Use GaussianProcessRegressor
        self.GP = []
        self.N = 5
        for _ in range(self.N**2):
            self.GP.append(GaussianProcessRegressor(kernel=kernel_matern + kernel_linear, optimizer='fmin_l_bfgs_b'))


    def Add_region(self, data, targets, Bool = True):
        '''
        Division of data into 25 regions
        '''
        x = data[:, 0]
        y = data[:, 1]
        ...
        x_range = (x_max - x_min) / self.N
        y_range = (y_max - y_min) / self.N
        ...
        X = [[x_min, x_min + x_range + delta_x]]
        Y = [[y_min, y_min + y_range + delta_y]]
        if self.N > 2:
          for i in range(self.N-2):
            X.append([x_min + (i+1)*x_range-delta_x, x_min + (i+2)*x_range+delta_x])
            Y.append([y_min + (i+1)*y_range-delta_y, y_min + (i+2)*y_range+delta_y])
        X.append([x_min + (self.N-1)*x_range-delta_x, x_max])
        Y.append([y_min + (self.N-1)*y_range-delta_y, y_max])
        ...
        for i in range(len(data)):
            ...
            for ix in range(len(X)):
                if not Bool and found:
                    break
                for iy in range(len(Y)):
                    if X[ix][0] <= x[i] and x[i] <= X[ix][1] and Y[iy][0] <= y[i] and y[i] <= Y[iy][1]:
                        point_regions.append(iy*len(X)+ix)
                        if not Bool:
                            found = True
                            break
            ...
            for region in point_regions:
                regions.append([data[i, 0], data[i, 1], targets[i], region])

        regions = np.array(regions)
        return regions[:, :2], regions[:,2], regions[:, 3]
    ...
    def generate_predictions(self, test_coordinates: np.ndarray, test_area_flags: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        test_coordinates, _, regions = self.Add_region(test_coordinates, np.zeros(test_coordinates.shape[0]), False)
        for i in range(self.N**2):
            index = regions == i
            aux_mean, aux_std = self.GP[i].predict(test_coordinates[index, :], return_std=True)
            gp_mean[index]=aux_mean
            gp_std[index]=aux_std

        # TODO: Use the GP posterior to form your predictions here

        # predictions = gp_mean+3.2*(gp_std+ 1)*test_area_flags
        predictions = np.zeros(test_coordinates.shape[0], dtype=float)
        for i in range(test_coordinates.shape[0]):
            samples = self.rng.normal(loc=gp_mean[i], scale=gp_std[i], size=30)
            costs = np.array([calculate_cost(np.array([gp_mean[i]]), np.array([sample]), np.array([test_area_flags[i]])) for sample in samples])
            predictions[i] = samples[np.argmin(costs)]
    ...
    def train_model(self, train_targets: np.ndarray, train_coordinates: np.ndarray, train_area_flags: np.ndarray):
        ...
        train_coordinates, train_targets, regions = self.Add_region(train_coordinates, train_targets, True)

        for i in range(self.N**2):
            index = regions == i
            self.GP[i].fit(train_coordinates[index, :], train_targets[index])
